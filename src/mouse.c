...
static uint8_t mouse_r = 0;
static uint8_t mouse_c = 0;
...
static bool LeftBtnPressed(int16_t x, int16_t y)
{
    ...
    if (txtbox != NULL && r < canvas_rows()-1) { // not in status bar
        // move the cursor to the current mouse position
        uint16_t max_r = txtbox->doc.last_row;
        txtbox->doc.cursor_r = txtbox->doc.offset_r + r - txtbox->r;
        if (txtbox->doc.cursor_r > max_r) {
            txtbox->doc.cursor_r = max_r;
        }

        uint8_t max_c = txtbox->doc.rows[r-txtbox->r].len;
        txtbox->doc.cursor_c = c - txtbox->c;
        if (txtbox->doc.cursor_c > max_c) {
            txtbox->doc.cursor_c = max_c;
        }
        UpdateCursor();
        UpdateStatusBarPos();
    }
...
static bool LeftBtnReleased(int16_t x, int16_t y)
{
    ...
    //       x, y, y/display.font_height(), x/display.font_width());
    return true;
}
    ...
    if (popup != NULL) {
        ...
        if (panel_type == MSGDIALOG) {
            ...
            if (r == (txtbox->r) &&
                ...
                for (uint8_t i = 0; i < dlg->panel.num_btns; i++) {
